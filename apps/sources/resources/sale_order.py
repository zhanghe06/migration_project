...
from apps.sources.outputs.sale_order import fields_item_sale_order, fields_item_sale_order_cn
from apps.sources.reqparsers.sale_order import (
    structure_key_item,
    structure_key_items,
...
from apps.sources.apis.sale_order import (
...
class SaleOrderResource(Resource):
    ...
    def get(self, pk):
        """
        Example:
            curl http://0.0.0.0:5000/sources/sale_order/1
        :param pk:
        :return:
        """
    ...
    def delete(self, pk):
        """
        Example:
            curl http://0.0.0.0:5000/sources/sale_order/1 -X DELETE
        :param pk:
        :return:
        """
...
class SaleOrderListResource(Resource):
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:5000/sources/sale_orders
            curl http://0.0.0.0:5000/sources/sale_orders?last_pk=1000&limit_num=2
        :return:
        """
        # 条件参数
...
class SaleOrderPaginationResource(Resource):
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:5000/sources/sale_orders/pagination
            curl http://0.0.0.0:5000/sources/sale_orders/pagination?page=2000&per_page=2
        :return:
        """
        # 条件参数
        filter_parser = reqparse.RequestParser(bundle_errors=True)
