...
from apps.sources.outputs.sale_delivery import fields_item_sale_delivery, fields_item_sale_delivery_cn
from apps.sources.reqparsers.sale_delivery import (
    structure_key_item,
    structure_key_items,
...
class SaleDeliveryResource(Resource):
    ...
    def get(self, pk):
        """
        Example:
            curl http://0.0.0.0:5000/sources/sale_delivery/1
        :param pk:
        :return:
        """
    ...
    def delete(self, pk):
        """
        Example:
            curl http://0.0.0.0:5000/sources/sale_delivery/1 -X DELETE
        :param pk:
        :return:
        """
...
class SaleDeliveryListResource(Resource):
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:5000/sources/sale_deliveries
            curl http://0.0.0.0:5000/sources/sale_deliveries?last_pk=1000&limit_num=2
        :return:
        """
        # 条件参数
...
class SaleDeliveryPaginationResource(Resource):
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:5000/sources/sale_deliveries/pagination
            curl http://0.0.0.0:5000/sources/sale_deliveries/pagination?page=2000&per_page=2
        :return:
        """
        # 条件参数
        filter_parser = reqparse.RequestParser(bundle_errors=True)
